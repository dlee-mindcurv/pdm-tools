...
description: "Start autonomous PRD execution using PDM Ralph. Loops through user stories in prd.json until all pass or max iterations reached."
---

# PDM Ralph Execute

Launches the PDM Ralph orchestrator to autonomously execute all user stories in a feature's prd.json.
...
```
/pdm-ralph-execute <feature-id>
```

**Example:**
```
/pdm-ralph-execute add-logo
```
...
2. **prd.json exists**: Run `/pdm-create-prd-json <feature-id>` first if needed
...
```bash
# Count incomplete stories (using Node.js instead of jq)
node -e "const d=require('./product-development/features/$FEATURE_ID/prd.json'); console.log(d.userStories.filter(s=>!s.passes).length)"

# Show incomplete story IDs
node -e "const d=require('./product-development/features/$FEATURE_ID/prd.json'); d.userStories.filter(s=>!s.passes).forEach(s=>console.log(s.id+': '+s.title))"
```

### Step 3: Launch PDM Ralph
...
Launching PDM Ralph orchestrator...

[PDM Ralph executes iterations...]
